                ...
                if ($item->ipaddr != 'remote-ical-file')
                {
                    ...
                    foreach($data["apps"] as $app)
                           ...
                           )
                        {
                            ...
                            echo "BEGIN:VEVENT\n";
                            echo "DTSTART".esc_html(($icalSettings[0]->cal_tzid != '' ? ';TZID='.$icalSettings[0]->cal_tzid : '').":".gmdate("Ymd",strtotime($datetime.$icalSettings[0]->cal_time_zone_modify))."T".gmdate("His",strtotime($datetime.$icalSettings[0]->cal_time_zone_modify)))."Z\n";
                            echo "DTEND".esc_html(($icalSettings[0]->cal_tzid != '' ? ';TZID='.$icalSettings[0]->cal_tzid : '').":".gmdate("Ymd",strtotime($datetime.$icalSettings[0]->cal_time_zone_modify.$duration))."T".gmdate("His",strtotime($datetime.$icalSettings[0]->cal_time_zone_modify.$duration)))."Z\n";
                            echo "DTSTAMP".esc_html(($icalSettings[0]->cal_tzid != '' ? ';TZID='.$icalSettings[0]->cal_tzid : '').":".gmdate("Ymd",$submissiontime)."T".gmdate("His",$submissiontime))."Z\n";
                            echo "UID:uid".esc_html($item->id.'_'.$ct."@".$_SERVER["SERVER_NAME"].($submissiontime > $updatefeaturetime?$form_id:''))."\n";
                            echo "DESCRIPTION:".esc_html($base_description)."\n";
                            ...
                            echo "SUMMARY:".esc_html($base_summary)."\n";
                            echo "TRANSP:OPAQUE\n";
                            echo "END:VEVENT\n";
                        }
                }
